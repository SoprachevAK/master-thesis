...
Была разработана гибкая микросервисная архитектура с применением Serverless подхода, которая позволила существенно оптимизировать инфраструктурные затраты, упростить развёртывание и тестирование приложения, с применением подхода было реализовано шесть микросервисов:
...
- Map-storage -- отвечает за управления картами, их хранение и инвалидацию кеша.
...
Применение Serverless подхода позволяет гибко горизонтально и географически масштабироваться адаптируясь к переменной нагрузке. Для оптимизации скорости скачивания карты с серверов была применена сеть доставки контента (CDN) с автоматический инвалидацией кеша при обновлениях карты.
