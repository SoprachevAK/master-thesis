...
При этом заказчик и художник могут быть одним и тем же человеком, так как в большинстве случаев заказчик сам будет оцифровывать карту. Однако в некоторых случаях заказчик может поручить эту задачу художнику как своему представителю, так и заказать отрисовку карты у PolyMap.
...
Подробная реализация будет рассмотрена в #ref(<web-map-implementation>, supplement: "разделе").
...
На текущий момент существует три основных рантайма выполнения `JavaScript`:
