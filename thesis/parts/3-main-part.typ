...
Проблема навигации в целом и по внутренним кампусам и помещениям известна давно и есть множество предложений по её решению. Глобально можно выделить два подхода к решению этой проблемы:
+ Аналоговый подход. В него входит улучшение оффлайн среды: установка указателей, информационных стендов, печатных карт, цветового кодирования и т.д.
+ Цифровой подход. Более продвинутый и современный способ, лучше подходит для молодых поколений, которые привыкли к цифровым решениям. В него входит использование мобильных приложений, веб-сайтов, интерактивных стендов и т.д. Цифровые решения можно подразделить ещё на две категории, а зависимости от того, кем они разработаны
  + Разработки крупных корпорациями. Сюда относятся приложения карт такие как Google Maps, Яндекс.Карты, 2GIS. Они ориентируются на массовую аудиторию и в первую очередь на ориентацию уровня города. Несмотря на то, что они поддерживают навигацию по отдельным зданиям, размещение карт по своему желанию в них невозможно. Корпорации сами выбирают какие здания и территории будут отображаться на их картах. Кроме того, они крайне плохо поддерживают навигацию внутри зданий. Это направление не является приоритетным для них.
  + Разработки небольших стартапов. В отличие от корпораций, стартапы наоборот ориентируются на проблемы заказчиков и стараются предоставлять сервис для решение конкретных узких задач. В контексте разработки карт, стартапы предлагают услуги по размещению интерактивных планировок зданий, и любой желающий заказчик, может воспользоваться их услугами. Именно к этой категории относится проект PolyMap.

Для наиболее комфортной навигации, оптимально будет использовать оба подхода, однако в современных условиях тотальной цифровизации, новым поколения воспринимают цифровые решения как более удобные и привычные, нежели аналоговые.

== Обзор существующих решений
Несмотря на то, что платформа PolyMap относится к категории цифрового подхода стартапов, будет полезно рассмотреть предметную область в целом, это позволит оценить и перенять часть идей из смежных подход.

=== Аналоговый подход
Аналоговый подход достаточно обширен, он начинается ещё на этапе планировки зданий, и продолжается в процессе их эксплуатации. На этапе проектирования зданий, необходимо продумать понятные коридоры, лестницы, выходы. Наиболее удобными являются прямоугольные здание, например вокзалы, Т образные и H образные планировки, когда помещения строятся вокруг простых структур коридоров. Так же, важным фактором является возможность добраться до любого места на этаже не покидая этот этаж и правильная нумерация кабинетов, которая включает в себя номер этажа, например кабинет #text(weight: "bold")[1]12 находится на первом этаже, а #text(weight: "bold")[2]12 на втором.

К примером хорошей планировки можно отнести Главное учебное здание СПбПУ, оно представляет классическую H образную форму коридоров. Планировка здания на @main-building, белым цветом отображаются коридоры.
...
Примером неудачной планировки является Научно-исследовательский корпус СПбПУ, он представляет собой сложную структуру с множеством коридоров и переходов, попасть в одну часть здания из другой можно только через лестничный пролёт, что вводит в заблуждение и затрудняет навигацию.
...
Цветовое кодирование (colorcoding) -- это метод, который позволяет выделить важные элементы с помощью разных цветов. Например, с помощью цветом можно выделить разные этажи, или разные категории аудиторий, что позволит легче ориентироваться и проще запомнить расположение помещений @Alansari2022WayfindingIA. Похожее решение можно будет применить и цифровых картах, например, выделять аннотации и типы кабинетов разными цветами.
...
Цифровой подход корпораций заключается в разработке карт масштаба городов и стран, которые ориентируются на массовое потребление. Многие их них включает функционал планировки помещений, однако компании не дают возможности размещать на своих картах произвольные планировки и выставляют жесткие требования к тем помещениям, которые они размещают самостоятельно. Здания должны быть публичны и часто посещаемы, в основном, под такие требования подходят аэропорты, вокзалы и крупные торговые центры. Делать планировку университетов или конференц-залов, которые используются ограниченным кругом лиц, корпорациям невыгодно.
...
==== Apple Maps
Apple Maps -- это картографический сервис от компании Apple, представлен на iOS и MacOS. На данный момент является наиболее качественным картографическим сервисом в техническом плане.
...
В июле 2024 года стала доступна веб-версия Apple Maps в режиме беты с сильными ограничениями, с тех пор обновлений не было, и по текущий момент карты доступны в беда версии. Планировок помещений нет. Реальной ширины дорог и разметки нет.

#subfig(
  figure(image("../assets/alternatives/apple.jpg"), caption: [Карты территорий]),
...
За последние годы Google Maps выпустили несколько крупных обновлений, одна из которых -- поддержка дорог реальной ширины, а так же разметка на дорогах, как и в Apple Maps.
...
#par(first-line-indent: 0pt)[Недостатки:]
- Вместо 2D проекции, используется вид сверху на 3D, что неудобно, так как создаёт перспективные искажения, особенно вокруг высоких зданий (небоскрёбов). В настройках карт можно в принципе отключить объёмные здания, и это решает проблему, однако отключает их и в 3D режиме тоже.
- Нет возможности размещать свои карты
- Маршруты внутри помещений не строятся
...
- Нет возможности размещать свои карты
- Маршруты внутри помещений не строятся (было анонсировано, не работает)
- Реальная ширина дорог доступна только в режиме автомобильного навигатора

#subfig(
  figure(image("../assets/alternatives/yandex.jpg"), caption: [Карты территорий]),
...
В декабре 2022 года, Яндекс анонсировал новое поколения Яндекс Карт, с высокой детализацией, в полноценном 3D режиме и хорошо развитой пешеходной средой. Однако, на настоящий момент (май 2025 года), новое поколение карт так и не появилось, никаких новостей больше не было. Часть функционала, такая как разметка на дорогах, была добавлена в старую версию карт.
#fig("Новое поколение Яндекс Карт", "../assets/alternatives/new-yandex.png", width: 100%)

=== Цифровой подход стартапов

В отличие от крупных корпораций, стартапы намного более гибкие и клиентоориентированные. Существует множество картографический сервисов, которые созданы небольшими компаниями для решения узкого спектра задач. Рассмотрим те из них, которые ориентируются на планировку помещений и прилежащей территории.

Значимой проблемой практически всех карт, является низкое качество самих карт, медленно работают, зависают, сделаны поверх библиотек, которые сильно ограничивают возможность наложения. Кроме того, многие фокусируются только на планировках одного здания, и не поддерживают навигацию по территории. У многих компаний, рекламируемый результат не соответствует действительности, а найти реальный пример их карт достаточно сложно.
...
- Мало примитивов для создания карт (стены, двери, окна). Нет лестниц, нет возможность разметить помещения по категориям
- Устаревший дизайн
...
- Повышенный спрос на карту университетов в начале каждого дня. По утрам, в 10:00, 12:00 и 14:00, перед началом пар, студенты будут открывать карту, чтобы найти нужную аудиторию. Что подтверждается статистикой использования пилотной версии (@appmetrika-day).
#fig("График использования PolyMap в течении дня", "../assets/appmetrika-day.png", width: 100%)<appmetrika-day>
...
== Serverless подход
Для оркестрации микросервисов наиболее распространено использовать Kubernetes @Vano2023CloudNativeWO @Schneider2020ACN, однако последние несколько лет, растёт популярность альтернативного подхода -- Serverless. Он позволяет запускать и масштабировать микросервисы без необходимости взаимодействовать с виртуальными или выделенными серверами @Tari2024AutoscalingMI. Вместо этого, Serverless предлагает арендовать у облачных провайдеров только вычислительные ресурсы, которые фактически были использованы.
...
+ Низкая стоимость -- Serverless распространяется по модели pay-as-you-go, то есть необходимо оплачивать только те ресурсы, которые были фактически использованы, что хорошо видно на @serverless-vs-dedicated. Это позволяет значительно снизить затраты на инфраструктуру @soprachevServerless.
...
Стоит отметить, что все эти провайдеры соответствуют требованиям российского законодательства о хранение данных (ФЗ-152), что является важным фактором при выборе облачного провайдера @Яхонтова2025СравнительныйАР.
...
- Serverless Container -- позволяет запускать Docker @docker контейнеры в ответ различные события.
...
Этот микросервис крайне важен для SEO оптимизации карты, веб-версия которой является Single Page Application (SPA), что усложняет классические методики SEO @Kowalczyk2024EnhancingSI, он занимается генерацией index.html страницы для web-версии карт. В зависимости от URL запроса, он будет добавлять в сгенерированный UI фреймворком index.html файл, мета-теги, которые будут использоваться поисковыми движками для индексации страницы, а так же обслуживать нужны поисковых движков, возвращая корректные
...
В качестве хранилища системы контроля версий Git @git можно использовать несколько систем:
- GitHub @github
- GitLab @gitlab
- Bitbucket @bitbucket
...
Карта является однострочным веб-приложение (SPA), и имеет много интерактивных элементов, что означает, что для её реализации необходимо использовать веб-фреймворк, в виду того, что карта будет активно обновляться и развиваться, к этому выбору стоит подойти с особой тщательностью. К основным фреймворкам, которые специализируются на разработке SPA относятся:
...
- Стабильность API -- устойчивость фреймворка в долгосрочной перспективе, совместимость версий. Отрытый или закрытый проект.
...
С использованием 5 алгоритмов был рассчитан общий балл для каждого облачного провайдера. В @sppr-framework-report находится подробный отчёт СППР.

#figure(caption: [Результат работы СППР выбора фрейморка])[
  #show table.cell.where(y: 0): set text(weight: "bold")
  #table(
    columns: (auto, auto, auto, auto, auto, auto, auto, auto),
    align: horizon,
...
+ В видеокарту загружается буффер вершин (vertex buffer), который может быть индексированным или не индексированным. Индексированный буфер состоит из массива параметров вершин и индексов, которые указывает на вершины для объединения в треугольники. У каждой вершины может быть сколько угодно параметров, которые попадут на вход вершинного шейдера.
...
- `Occupant Annotation` -- самая базовая аннотация, с её помощью отображаются предназначения кабинетов внутри здания. Например номер лектория, название столовой, конферец-зала, и т.д.
- `Amenity Annotation` -- аннотация, которая отображает дополнительные удобства внутри здания. Например, лестницы, лифты, входы и выходы, а так же удобства вне здания, такие как парковки, велопарковки, спортивные площадки и т.д.
...
Процедурная отрисовка графики на процессоре медленнее чем отрисовка графики на видеокарте, особенно медленной операцией является отрисовка текста. Результат такой отрисовки будет в разы качественнее, чем любая альтернативна на видеокарте, именно процессорные алгоритмы позволяют получить наиболее качественное сглаживание. При масштабировании карты, аннотации двигаются друг относилтельно друга, а это означает, что каждую аннотацию нужно каждый кадр перерисоваывать в новых координатах.
...
Для оптимизации процесса отрисовки было применено кеширование (запекание) аннотаций и текстовых подписей в текстурные атласы (@annotation-atlas). При повтороной перерисовке, аннотация не выполняет медленную отрисовку текста и картинки, а просто копирует область пикселей из текстурного атласа в нужные координаты целевого холста (@baked-render), такое копирование является крайне быстрой операцией. Если места на аталасе недостаточно, то создаётся новый. Красной рамной на @annotation-atlas и @baked-render отображаются границы отдельных областей на атласе. У аннотаций в активном и закреплённом состоянии, а так же, во время анимации перехода между состояниями, отрисовка происходит в обычном режиме, так как в таких состояниях не отображаются много аннотаций одновременно, и медленная отрисовка не будет заметна пользователю. Кешировать необходимо талько те состояния, которые отображаются на экране в больших количествах.
...
Введём две велечины: собственная частота без затухания $omega = sqrt(k)$ (частота отклика), коэффициент демпфирования: $zeta = c/(2 sqrt(k))$
...
Примеры работы функции пружинной анимации приведены на @byDamping и @byFrequency. На них показано, как меняется кривая анимации в зависимости от изменения коэффициента демпфирования и частоты отклика. Как видно из графиков, продолжительность анимации зависит от частоты отклика, а амплитуда колебаний ("пружинность") от коэффициента демпфирования. При передачи функции интерполяции в твиннер, необходимо учитывать, что пружинная анимация является бесконечно затухающей, а значит, нужно подбирать время анимации так, чтоб обрезать колебания в моменте, когда они становятся менее одного пикселя.

#import "../assets/spring-animatino-plot.typ": byDamping, byFrequency

#figure(caption: [Изменение коэффициент демпфирования $ζ$ при частоте отклика $ω = 0.5$])[
  #byDamping
]<byDamping>

#figure(caption: [Изменение частоты отклика $ω$ при коэффициент демпфирования $ζ = 0.3$])[
  #byFrequency
]<byFrequency>
...
Это позволяет снизить число вершин в графе и ускорить поиск, даже если на одной карте будет много зданий, то при поиске маршрута будут учитываться только граф вне здания, и граф внутри целевого здания. Если здание имеет несколько входов, то будут перепробованы все входы, и отображаться будет наиболее оптимальный маршрут. Отображение маршрута происходит в системе аннотаций на `Canvas`, так как маршрут всего один, значительную нагрузку на CPU он не оказывает и в оптимизации отрисовки не нуждается.
...
=== Хостинг
Несмотря на то, что веб-карта является веб приложением (фронтентом), его всё ещё необходимо хранить на сервере и раздавать пользователям по GET запросам на домен карты. В выбранной инфраструктуре Yandex Cloud, для хостинга статичных сайтов, можно использовать Yandex Object Storage (S3). Он позволяет настроить баккет в режиме HTTP сервера (@s3-web-hosting), в этом случае, баккет станет доступен из интернета по служебному домену, после чего, в настройках DNS нужно будет добавить CNAME запись, которая будет указывать на баккет (@dns-s3-cname). В этом случае даже не нужно использовать API Gateway, так как S3 будет раздавать статичные файлы по HTTP запросам.
...
Что бы не создавать излишнюю нагрузку на Object Storage (каждый запрос тарифицируется), рационально использовать CDN, для этого нужно загружать статичные файлы а баккет с указанием заголовков `Cache-Control`, в этом случае, Object Storage будет добавлять эти заголовки к ответам, и CDN будет кешировать файлы.
...
Такой хостинг получается крайне дешевым, а в большинстве случаев и беслпатным, так как используемые ресурсы не превышают бесплатный лимит. При этом, Object Storage гарантирует высокую доступность и отказоустойчивость, а так же возможность масштабирования. В дополнение к этому, Yandex Cloud предоставляет продвинутый мониторинг полностью бесплатно, он настроен по умолчанию и позволяет отслеживать количество запросов, ошибки, время ответа.
...
=== Результат
В результате разработки была получено веб-приложение для отображения интерактивной карты, которое позволяет пользователю:
...
Приложение полностью адаптировано под мобильные устройства, и позволяет пользователю взаимодействовать с картой с помощью жестов, таких как прокрутка, масштабирование и поворот, а так же пользоваться пользовательским интерфейсом. На компьютреной версии, интерфейс отображается в виде прямоугольной панели в левой части экрана, а на мобильной версии, интерфейс отображается в виде плавающей панели внизу экрана, которую можно вытянуть вверх на весь экран, свернуть вниз или оставить по середине экрана, как на #ref(<polymap-mobile-web>, supplement: "рисунке").
...
== Серверная часть
В сервисе используется микросервисная архитектура, и благодаря хорошей декомпозиции (@microsrvices), реализацтя каждого микросервиа в отдельности не представляет сложности и не обладает уникальными особенностями, о которых можно было бы рассказать. Однако, есть некоторые общие аспекты, которые применимы ко всем реализованным микросервисам. Рассмотрим их в этом разделе.

=== Стек технологий<serverless-technology-stack>
Как упомяналось ранее, одним из ограничений Serverless подхода, является требование к быстрому холодному старту (время между началом запуска Docker контейнера и моментом, когда сервер будет готов слушать указанный порт). Это создаёт дополнительные нестандартные условия при выборе технологического стека. Благодаря использованию Serverless Containers, можно использовать любой стек который подойдёт для реализации, не ограничиваяс поддерживаемыми языками облака.

Наиболее популярные стеки для бекенда в 2025 году:
- Java + Spring Boot -- постепенно теряет популярность, высокое время холодного стандарта
- C\# + ASP.NET Core -- популярный стек, высокое время холодного стандарта
...
В качетсве языка программирования для бекенда выбран `TypeScript`, это крайне популярный язык, у него много специалистов, хорошая поддержка, наибольшее число библиотек, он статически типизированный и обладает комфортным синтаксисом. При работе с бекендом, запускать `TypeScript` код принято через `Node.js` предварительно скомпилировав его в `JavaScript`. Существует множество фреймворков для разработки веб-сервера на `Node.js`. Благодаря хорошей модульности и изолированности частей проекта, никаких специфичных требований для построения больших архитектур к фремворку не предьявляется. Основной критерий выбора -- скорость холодного старта и поддержка сообщества (популярность).
...
Было принято решение попробовать использовать современный и многообещающий `Bun`. Производительность достигается за счёт использования движка JavaScriptCore разработаного Apple для Safari, движок с открытым исходным кодом, а так же, за счёт использования язфка программирования Zig с применением низкоуровневых оптимизаций. Кроме того, Bun представляет из себя инструмент всё в одном, который включает в себя пакетный менеджер, сборщик, рантайм имеет поддержку тестов, запускает TypeScript и JSX из коробки, имеет ряд полезных встроенных библиотек (PostgresSQL driver, SQLite driver, S3 Cloud Storage driver, Redis client, WebSocket server (including pub/sub), HTTP server, HTTP router), каждая из этих библиотек является прослойкой к низкоуровневому и очень хорошо оптимизированному коду на Zig, в результате, все базовые возможности, работает в разы быстрее аналогов JS библиотек для Node или Deno @Ostretsova2025COMPARATIVEAO.

Рантайм практически полностью совместим с API Node.js и подавляющее число Node.js библиотек будут корректно работать в Bun, все популярные библиотеки адаптированы под Bun, кроме того, вокруг Bun сложилась экосистема библиотек и инструментов, которые были разработаны с философией максимальной производительности, и достигают этого за счёт использования высоко оптимизировннаый API Bun.

#include "../assets/bun-node-deno.typ"

Кроме рантайма нужно ещё выбрать фреймворк для удобста создание REST API сервера, большинство фреймворков предоставляют очень похожый синтаксис, так что наиболее важным критерием является производительность и скорость холодного старта. Наиболее популярные быстрые фреймворки:
...
- Elysia -- быстрый новый фреймворк, разаработанный для Bun

Для выбора фрейморка, было проведено тестирование на предмет времени ответа в разных сценариях для холодного старта и горячего. Тестирование проводилось на реальной инфраструкторе Yandex Cloud, с использованием Serverless Containers. Результаты тестированния приведены в @serverless-cold-start-framework.

В ходе тестирования, было выяснено, что кроме фрейморка и рантайма, на время холодного старта влияет базовый образ Docker контейнера (а точнее его размер), а так же, применение бандлера и сам бандлер. Бандлер позволяет собрать весь исходный код в один файл (в том числе библиотеки), а так же минимизировать его. Это позволяет сократить время загрузки и парсинга исходного кода. Современные бандлеры используют механизм Tree Shaking, который позволяет удалить неиспользуемый код из бандла, что крайне эффективно при использовании библиотек. При сравнение были проведены тесты с ипользованием разных бандлеров (ESBuild, Parcel, Rollup, Webpack, Bun bundler) и без них. Тесты проводились на микросервисе генерации QR кодов и на тестовом микросервисе с большим числом библиотек, что делает результаты более репрезентативными.
...
=== Шаблон микросервиса
Для удобста создания микросервисов, был создан шаблон микросервиса, который можно копиторить и использовать в качестве основы для нового микросервиса. Шаблон включает в себя:
...
- Настроенный GitHub Actions, плробнее описан в @ci-cd-pipeline
...
- Настроенные ESLint @eslint для оценки качества кода, подробнее в @code-quality-check
- Настроенные модульные тесты с использованием `bun test`, подробнее в @unit-tests
- Настроенные Cypress E2E тесты, подробнее в @e2e-tests
...
После чего, программисту, остаётся только писать реализацию кода, модульные и E2E тесты, и вносить изменения в OpenAPI спецификацию. Все настройки, конфигурации и инфраструктура уже готова в шаблоне и не требует дополнительных действий. Такой подход значительно ускоряет разработку, не требует повторения рутинных действий, и гаратирует единообразие кода по всему проекту.

=== Раздача карта и CDN
Одной из особенностей архитектуры PolyMap, является оптимизация раздачи статический файлов через CDN. В случае с фронтендом, фреймворк Vue, в момент компиляции генерирует скрипты, с названиями в виде хуш-функции от их содержимого, что делает их полностью статическими файлами, никаких сложностей с использованием CDN не возникает, эти файлы хранятся в Yandex Object Storage с указанием заголовков `Cache-Control`, на время которого и кешируются в CDN. При обновлении фронтенда, случается его перекомпиляция, и изменённые файлы результата имеют другое название, что позволяет отличить их от старых закеширвованных версий.

Однако, кроме фронтенда, значимое потребление трафика происходит при раздаче файлов карт, этот процесс тоже оптимизирован через CDN, но карты не являются полностью статическими файлами. В случае изменения (обновления) карты, название файла остаётся прежним, и необходит способ максимально быстро достваить новую карту до пользователей. Один из подходов сделать это -- использовать короткое время кеширования, например 1 час, в этом случае, при обновлении карты, пользователи получат новую версию в течение одного часа после обновления. Такой подход решит проблему, но создат излишнюю нагрузку на сервер и замедлит часть запросов, которые будут идти в обходи кеша, в большинстве случаев, карты меняются сильно реже одного часа. По этому, в PolyMap используется другой подход, при обновлении карты: после обновления, микросервис `map-storage`, который отвечает за управление картами, отпрвляет в CDN систему по API запрос с указанием URL карты, кеш которой нужно инвалидировать. CDN система удаляет кеш для этого URL, и при следующем запросе, будет получена и закеширована новая версия карты. Это позволяет мгновенно доставлять до пользователей обновления, при этом оставляя большое время кеширования.
...
Мониторинг -- важная часть любого программного продукта, крайне важно не только опубликовать систему, а ещё и отслеживать её состояние, время ответов, ошибки, число пользователей и нагрузку на систему. В случае возникновения ошибок, полезно иметь логирование, по которму можно будет понять, что именно привело к ошибке.
...
=== Алертинг
Сервис мониторинга позволяет настраивать уведомления на достижение пороговых значений метрик. Уведомления могут приходить через различные каналы: email, Telegram, SMS. Алертинг позволяет максимально быстро регировать на нестандартное поведение системы, совевременно исправлять ошибки и узнавать о неожиданных ситуациях.

На @yandex-alerting показан пример уведомления от Yandex Monitoring о превышении порога числа запросов в секунду на сервис API Gateway. Превышение было достигнуто во время проведения нагрузочного тестирования. Уведомление приходит в момент превышение порога, а так же, после нормализации. В отчёте о нормализации, можно увидеть график числа запросов, время когда он превысил установелнный порог и вернулся обратно в норму.
...
Рассмотрены некоторые детали реализации основного функционала проекта -- веб-версии карты, описан процесс батчинга геометрии для отпимизации GPU, рассмотрен процесс запекания аннотаций, а так же их система пружинных анимаций, описан механизм достижения энергопотребления с помощью динамической частоты кадров. Описан хостинг статичных файлов в Yandex Object Storage, с использованием CDN для оптимизации раздачи файлов и система инвалидации.
...
=== Итоговый CI/CD Pipeline в GutHub Actions<ci-cd-pipeline>
...
- ESLint -- статический анализатор кода для JavaScript и TypeScript, позволяет проверять код на соответствие стандартам, на наличие ошибок и потенциальных проблем. В PolyMap во всех проектах используется ESLint c настройками @eslintts:
...
+ Тестирование на уровне компонентов (Component testing) -- это тестирование отдельных компонентов пользовательского интерфейса, в случае, если используется компонентный UI фреймворк. В случае с PolyMap, это VueJS, который является компоненты и поддерживает компонентное тестирования.
+ Тестирование визуального интерфейса (Visual testing) -- это тестирование наиболее приближенное к реальному пользовательскому опыту, оно проходится по сценарием использования, и на каждом шаге делает скриншот приложения, который сравнивается с эталонным. В случае, если скриншот отличается от эталонного, то тест считается проваленным. При хорошем покрытии, этот подход позволяет гарантировать, что внесённые изменения повлияли только на ожидаемую часть интерфейса. Например, частой проблемой является внесение изменений в компьютерную версию сайта, которые ломают мобильную версию, однако, программист вносящий изменения, мог проверять функционал только на компьютерной версии. Визуальное тестирование позволит избежать таких ошибок.
...
Рассмотрим каждый их них подробнее с точки зрения пригодности для PolyMap, в частности нас интересует поддержка веб тестирования, компонентного для VueJS, возможность запуска тестов в Github CI/CD @ricca2024comparative.
...
Cypress был интегрирован в CI/CD процесс и запускается на каждый PR, отчёт о прохождение тестов выводится в результат Action'a, в случае проваленных тестов, прикладывается короткий видеоролик с выполнением неудачных тестов. Кроме того, Cypress используется и в некоторых микросервисах для тестирования визуального ответа, например в микросервисе `qr-generator`, который стилизует QR коды, Cypress позволяет проверить, что QR код отображается таким, каким он и ожидался (Visual testing). Пример отчёта о прохождении тестов (@pr-ci-report, раздел `Cypress Results`)
...
Нагрузочное тестирование -- это тестирование системы на предмет её производительности и устойчивости к нагрузке. В PolyMap используется бесконечно масштабируемый Serverless подход, и нагрузочное тестирование не имеет смысла, так как система будет масштабировать под любую нагрузку, и время ответа будет оставаться близким к константе. Однако, использование инструмента нагрузочного тестирования, позволяет убедиться в этом, а так же, проверить, как работает мониторинг и алерты в случае резкого увеличения нагрузки, подробнее об этом рассмотрено в разделе #ref(<monitoring>, supplement: none).
