...
+ Цифровой подход. Более продвинутый и современный способ, который лучше подходит для молодых поколений, привыкших к цифровым решениям. В него входит использование мобильных приложений, веб-сайтов, интерактивных стендов и т.д. Цифровые решения можно подразделить ещё на две категории, в зависимости от того, кем они разработаны:
  + Разработки крупных корпораций. Сюда относятся картографические приложения, такие как Google Maps, Яндекс.Карты, 2GIS. Они ориентированы на массовую аудиторию и в первую очередь на навигацию городского уровня. Несмотря на то, что они поддерживают навигацию по отдельным зданиям, размещение карт по своему желанию в них невозможно. Корпорации сами выбирают, какие здания и территории будут отображаться на их картах. Кроме того, они крайне плохо поддерживают навигацию внутри зданий. Это направление не является для них приоритетным.
...
Несмотря на то, что платформа PolyMap относится к категории цифрового подхода стартапов, будет полезно рассмотреть предметную область в целом — это позволит оценить и перенять часть идей из смежных подходов.

=== Аналоговый подход
Аналоговый подход достаточно обширен: он начинается ещё на этапе планировки зданий и продолжается в процессе их эксплуатации. На этапе проектирования зданий необходимо продумать понятные коридоры, лестницы, выходы. Наиболее удобными являются прямоугольные здания, например вокзалы, Т-образные и H-образные планировки, когда помещения строятся вокруг простых структур коридоров. Также важным фактором является возможность добраться до любого места на этаже, не покидая этот этаж, и правильная нумерация кабинетов, которая включает в себя номер этажа: например, кабинет #text(weight: "bold")[1]12 находится на первом этаже, а #text(weight: "bold")[2]12 — на втором.
...
==== Apple Maps
Apple Maps — это картографический сервис от компании Apple, представленный на iOS и MacOS. На данный момент является наиболее качественным картографическим сервисом в техническом плане.
...
В июле 2024 года стала доступна веб-версия Apple Maps в режиме беты с серьёзными ограничениями, с тех пор обновлений не было, и по текущий момент карты доступны в бета-версии. Планировок помещений нет. Реальной ширины дорог и разметки нет.
...
За последние годы Google Maps выпустили несколько крупных обновлений, одно из которых — поддержка дорог реальной ширины, а также разметки на дорогах, как и в Apple Maps.
...
В декабре 2022 года Яндекс анонсировал новое поколение Яндекс Карт с высокой детализацией, в полноценном 3D-режиме и хорошо развитой пешеходной средой. Однако на настоящий момент (май 2025 года) новое поколение карт так и не появилось, никаких новостей больше не было. Часть функционала, такая как разметка на дорогах, была добавлена в старую версию карт.
...
В этой главе будет рассмотрена верхнеуровневая архитектура сервиса PolyMap, будет выбран оптимальный подход к разработке серверной части, а также рассмотрен выбор оптимального облачного провайдера для реализации Serverless-архитектуры.
...
- *Заказчик* — человек, который заказывает разработку карты.
- *Пользователь* — человек, который использует уже созданную карту.
- *Художник* — человек, который по поручению заказчика оцифровывает карту в конструкторе сервиса.
...
Реализация веб-версии конструктора выходит за рамки текущей работы, однако планируется в дальнейших обновлениях сервиса, что накладывает ряд дополнительных требований на архитектуру:
...
== Serverless подход
Для оркестрации микросервисов наиболее распространено использовать Kubernetes @Vano2023CloudNativeWO @Schneider2020ACN, однако в последние несколько лет растёт популярность альтернативного подхода — Serverless. Он позволяет запускать и масштабировать микросервисы без необходимости взаимодействия с виртуальными или выделенными серверами @Tari2024AutoscalingMI. Вместо этого Serverless предлагает арендовать у облачных провайдеров только вычислительные ресурсы, которые фактически были использованы.
...
За счёт того, что микросервисы в Serverless-подходе запускаются только во время запроса пользователя и отключаются после завершения обработки, они потребляют ресурсы облака только в момент фактической работы, в отличие от Kubernetes, где вы арендуете виртуальные серверы вне зависимости от их использования. При переменной нагрузке Kubernetes будет простаивать значительное время, что приведёт к нерациональным затратам, которых позволяет избежать Serverless.
...
Кроме непосредственно Serverless-компонентов Яндекс Облако предоставляет и другие сервисы, которые будут полезны в проекте:
...
Также не стоит забывать, что кроме production-кластера необходимо будет поддерживать тестовый кластер для разработки и тестирования новых обновлений, что значительно увеличит затраты на инфраструктуру, в то время как в Serverless-подходе тестовые среды даже не будут выходить за пределы бесплатных квот. За три года активной разработки я ни разу не использовал больше 10% от выделенных квот, что позволяет полностью бесплатно разрабатывать и тестировать новые обновления.
...
Для поисковой оптимизации SPA с динамическим контентом принято использовать SSR-подход. При нём, на каждый запрос пользователя сервер запускает JavaScript-код приложения и создаёт полную HTML-страницу, после чего прикрепляет состояние фреймворка в виде JSON-объекта и отвечает пользователю. На стороне пользователя запускается процесс гидратации (hydration) — это процесс, при котором фреймворк берёт HTML-страницу и JSON-объект, и запускает приложение с уже готовым состоянием. Однако этот подход значительно увеличивает нагрузку на сервер, увеличивает скорость ответа, а также время до первого взаимодействия (Time to First Interaction — TTFI), что происходит из-за медленного процесса гидратации.
...
- iOS App, Android App, Web Map — нативные приложения, которые позволяют просматривать карты.
...
==== Dashboard-back
Этот микросервис будет реализован в будущем вместе с панелью управления. Он будет отвечать за взаимодействие с панелью управления, регистрацию заказчиков, хранение сервисных настроек карт, управление картами, расчёт аналитических графиков.
...
Визитной карточкой всего сервиса является веб-карта: именно с ней будет взаимодействовать большинство пользователей и именно с неё будет начинаться знакомство новых заказчиков с сервисом. Поэтому крайне важно реализовать её максимально качественно. Основная задача веб-карты — это динамически отображать карты разных кампусов в формате Extended-IMDF (баз разработан в рамках бакалаврской ВКР), а также предоставлять пользователям возможность взаимодействовать с картами, строить маршруты, открывать аннотации, осуществлять поиск по аннотациям. Важной задачей является сделать поддержку не только компьютерной, но и мобильной версии с адаптированным интерфейсом и управлением.
...
Исходя из того, что основным функционалом PolyMap является отображение карты, было принято решение не использовать сторонние библиотеки, а разработать своё решение, адаптированное под нужды и задачи PolyMap, которое будет наиболее эффективно выполнять задачи сервиса. Кроме того, собственное решение позволит не добавлять лишний функционал, который замедляет общую работу приложения.
...
- `Mesh` — меш, он объединяет `Material` и `Geometry`, с него и начинается процесс отрисовки.
...
После общей геометрии карты по значимости идут аннотации: они добавляют карте информативности, с их помощью подписываются здания, входы, парковки, номера кабинетов, столовые. По аннотациям пользователь сможет проводить поиск и строить маршруты между ними. В отличие от статичной геометрии карты, аннотации являются частью пользовательского интерфейса. Каждая аннотация привязана к точке на карте в экранном пространстве. Размер и горизонт аннотации являются неизменными при масштабировании и вращении карты. На аннотациях присутствует текст, по аннотациям пользователь может кликать. Все эти факторы накладывают серьёзные ограничения на систему отрисовки аннотаций.
...
Для оптимизации процесса отрисовки было применено кеширование (запекание) аннотаций и текстовых подписей в текстурные атласы (@annotation-atlas). При повторной перерисовке аннотация не выполняет медленную отрисовку текста и картинки, а просто копирует область пикселей из текстурного атласа в нужные координаты целевого холста (@baked-render): такое копирование является крайне быстрой операцией. Если места на атласе недостаточно, то создаётся новый. Красной рамкой на @annotation-atlas и @baked-render отображаются границы отдельных областей на атласе. У аннотаций в активном и закреплённом состоянии, а также во время анимации перехода между состояниями, отрисовка происходит в обычном режиме, так как в таких состояниях не отображается много аннотаций одновременно и медленная отрисовка не будет заметна пользователю. Кешировать необходимо только те состояния, которые отображаются на экране в больших количествах.
...
=== Хостинг
Несмотря на то, что веб-карта является веб-приложением (фронтендом), его всё ещё необходимо хранить на сервере и раздавать пользователям по GET-запросам на домен карты. В выбранной инфраструктуре Yandex Cloud для хостинга статичных сайтов можно использовать Yandex Object Storage (S3). Он позволяет настроить бакет в режиме HTTP-сервера (@s3-web-hosting), в этом случае бакет станет доступен из интернета по служебному домену, после чего в настройках DNS нужно будет добавить CNAME-запись, которая будет указывать на бакет (@dns-s3-cname). В этом случае даже не нужно использовать API Gateway, так как S3 будет раздавать статичные файлы по HTTP-запросам.
...
В качестве языка программирования для бэкенда выбран `TypeScript`. Это крайне популярный язык, у него много специалистов, хорошая поддержка, наибольшее число библиотек, он статически типизирован и обладает комфортным синтаксисом. При работе с бэкендом запускать `TypeScript`-код принято через `Node.js`, предварительно скомпилировав его в `JavaScript`. Существует множество фреймворков для разработки веб-сервера на `Node.js`. Благодаря хорошей модульности и изолированности частей проекта, никаких специфичных требований для построения больших архитектур к фреймворку не предъявляется. Основной критерий выбора — скорость холодного старта и поддержка сообщества (популярность).
...
- Настроенный GitHub Actions, подробнее описан в @ci-cd-pipeline
...
- Настроенный ESLint @eslint для оценки качества кода, подробнее в @code-quality-check
...
- Поддерживать структурированные JSON-логи
...
Рассмотрены некоторые детали реализации основного функционала проекта — веб-версии карты, описан процесс батчинга геометрии для оптимизации GPU, рассмотрен процесс запекания аннотаций, а также их система пружинных анимаций, описан механизм повышения энергоэффективности с помощью динамической частоты кадров. Описан хостинг статичных файлов в Yandex Object Storage с использованием CDN для оптимизации раздачи файлов и система инвалидации.
...
- На закрытие Pull Request'а — удаляет временное тестовое окружение
- На `push` в ветку `main` — публикует новую версию в производственном окружении
- На `push` в ветку `dev` — публикует новую версию в тестовое окружение
...
Этот процесс занимается применением уже принятых Pull Request'ов в производственное или тестовое окружение. Он запускается на `push` в ветку `main` или `dev`. Процесс упрощён, так как не требует резервирования директории и состоит всего из трёх шагов:
...
В случае с PolyMap могут быть применены оба подхода, однако было принято решение использовать только Visual testing, так как этот подход позволяет точно определить наличие нежелательного результата при применении изменений, а также существенно сокращает время на написание тестов. На начальных этапах разработки компонентное тестирование требует значительного времени на написание тестов, и при каждом изменении тесты придётся переписывать, что замедлит разработку. В случае с визуальным тестированием достаточно сделать несколько тестов основных пользовательских сценариев, и такие тесты представляют собой обычный скриншот приложения.
